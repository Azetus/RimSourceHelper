...
public class TypeRepository(SqliteConnection connection)
{
    ...
    {
        const string sql = """
            INSERT OR IGNORE INTO Types (Namespace, Name, FullName, BaseType, IsAbstract, IsInterface, IsEnum, IsSealed, IsCompilerGenerated, Accessibility, AssemblyName, AssemblyPath, SourceId)
            VALUES (@Namespace, @Name, @FullName, @BaseType, @IsAbstract, @IsInterface, @IsEnum, @IsSealed, @IsCompilerGenerated, @Accessibility, @AssemblyName, @AssemblyPath, @SourceId)
            """;
